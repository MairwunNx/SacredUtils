...
namespace SacredUtils.Resources.bin
{
    public class CheckAppAlphaUpdates
    {
            ...
            try
            {
                if (fileText.Contains("Automatically get and install alpha updates = true"))
                {
                    try
                    {
                        if (_connect)
                        {
                            const string appLatestVersionWeb = "https://drive.google.com/uc?export=download&id=1JnqBVN6qqftbhS_7xgkDFVgCqsb22A3o";
        ...
        public async Task GetSacredUtilsUpdateAsync()
        {
            ...
            const string appLatest = @"https://drive.google.com/uc?export=download&id=10yipgfZqHtfTCuf-zHGC6LBnFlp-ifrY";
